...
class CustomBlock extends Component {
    ...
    render() {
        const texts = {
            11: [
                ["Background",
                "The nodes of a neural network are based on the perceptron: a simple model which takes a number of inputs x1, x2, etc., multiplies them with a weight w1, w2, etc. and then adds a bias w0. The result of this function is then put into an activation function: don’t worry too much about that for now, just know that it scales the result to a value between 0 and 1. This scaling helps the network learn more quickly, as we will show in Level 4. You might notice that a neuron is just a mathematical function then, and that is indeed correct: in fact, a whole neural network is just a big, complex function with many weights and biases."],
                ["Your Task",
                "Your task here is quite simple: try to set the parameters of a neuron in such a way that the function maps best matches the data. This is actually just a linear regression, since the output of a single neuron will always be linear. Therefore, we are simply fitting a line to the data. We will ignore the activation function for now, so you better understand how it works."],
                ["The Data",
                "The data is generated along a line, with some random noise added."],
            ]
        }
