...
function App() {
  ...
  const apiEndpoint = window.location.origin + "/api/my_backend";
  ...
  useEffect(() => {
    let data;
    axios.get(window.location.origin + "/api/my_backend/?limit=1")
      .then((response) => {
  ...
  const postRequest = (e) => {
    ...
    setAccuracy(null);
    setIsTraining(1);
    axios.put(window.location.origin + "/api/my_backend/1", trainingData).then((response) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios.get(window.location.origin + "/api/my_backend/?limit=1")
      .then((response) => {
        ...
        axios.put(window.location.origin + "/api/my_backend/1", studentData)
          .then((response) => {
            console.log(response.status);
            fetchQueryResponse();
          })
